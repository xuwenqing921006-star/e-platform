...
### T-018: 若依工作概览真实闭环
- **陷阱**：若依默认首页包含框架介绍、技术选型、联系方式、捐赠等模板内容，若只接入菜单不替换首页，会直接偏离 A02 原型。
- **经验**：工作概览只需要一个聚合接口 `GET /api/admin/dashboard/summary`，由 service 组合内容、产品、账号和操作日志 mapper，前端首页只消费该契约 DTO。
- **避坑**：Dashboard service 如保留测试用 Clock 构造器，Spring 业务构造器需要明确 `@Autowired`，避免多构造器导致运行时装配不确定。
### T-019: 初始账号与产品样本数据真实闭环
- **陷阱**：PRD、Plan、契约和原型里的 112、52/60、阳光惠农贷只属于规则和示例，不能替代真实 Excel/CSV/产品清单；用户要求“选 10 条”时，如果仓库仍无真实源，也不能凭示例或臆造数据通过验收。
- **经验**：先用仓库文件搜索和测试记录数据源缺口，再只补幂等 seed 框架与账号扩展；产品数据未找到真实源时必须写明 BLOCKED，测试应证明没有把最小 seed 冒充 112 条真实产品。
- **本轮更新**：真实 Excel 到位后，按用户确认只抽取 10 条真实样本初始化，测试应同时校验 5 条涉农、5 条小微、固定银行映射和 `阳光惠农贷`，并在 Plan/审计文档中明确“不声明 112 条全量已初始化”。
- **避坑**：账号初始化不得写真实密码、JWT Secret 或生产库连接；可写 `cb_account_extension` 业务扩展 seed；产品 seed 只落库已确认的 7 个业务字段，Excel 中额度、期限、利率等内容只能保留在既有文本字段里，不得新增字段。

### T-020: 全部页面退出 Mock 联调审计
...
### Bugfix: H5 与后台乱码、卡片越界和若依默认入口残留
- **触发**：用户反馈 H5“乡村振兴与金融服务”服务指引卡片超出边界且列表乱码，后台所有页面乱码，并出现搜索、布局大小、公告通知等原型外若依模板入口。
- **根因**：Docker MySQL 初始化脚本未显式 `SET NAMES utf8mb4`，首次导入中文时客户端字符集错误，当前库中已写入 mojibake；H5 卡片缺少 `min-width: 0`、`overflow-wrap` 和溢出保护；若依后台只做了业务接口和部分页面替换，未把默认 header 工具、个人中心和默认菜单作为可执行审计项。
- **已有经验回查**：`T-016` 已提醒账号管理不能复用若依系统用户字段，`T-017` 已提醒操作日志不能暴露若依运维字段，`T-018` 已提醒默认首页必须替换，`T-020` 已提醒若依模板入口需要删除，`T-021` 已补充 Docker 初始化顺序。
- **为什么仍然犯错**：此前验收侧重 Mock 退出、接口契约和启动文档，没有直接查询 Docker MySQL 中的中文存储结果；审计脚本也没有覆盖后台 header 默认工具、角色菜单授权后的可见菜单集合、个人资料页字段和 H5 长文本溢出。
- **修复**：初始化 SQL 增加 `SET NAMES utf8mb4`，新增当前库 mojibake 修复 SQL 和后台菜单清理 SQL，并已对当前 Docker MySQL 执行；H5 卡片样式增加移动端硬换行和溢出约束；若依后台移除搜索、全屏、布局大小、公告通知、锁屏、个人中心、布局设置等原型外入口，个人资料页收敛为修改密码；审计脚本增加上述检查。
- **避坑规则**：包含中文 seed 的 Docker 初始化脚本必须声明客户端字符集，并在初始化后通过数据库查询或真实接口抽样验证中文；若依基座只允许保留已确认业务入口，默认 header 工具、系统菜单、个人资料字段和模板页面都要进审计；H5 列表卡片必须对长标题、异常字符串和窄屏做溢出回归。

### Bugfix: 后台登录、县域内容分类、多联系人和 H5 返回上下文
- **触发**：用户反馈后台登录页不需要验证码；内容新增应先选办公室再选分类，四县只能服务指引；金融产品需要最多 5 组业务经办人与联系方式；H5 详情返回不应固定回默认栏目。
- **根因**：若依默认验证码流程仍保留在前端登录页、注册页和后端登录服务；内容表单没有办公室优先联动，后端只做提交后拦截；金融产品 DTO 和表单只支持单联系人；H5 详情没有保存进入详情前的栏目上下文。
- **已有经验回查**：`T-013` 已要求县域办公室只能服务指引，`T-014` 已要求金融产品保持 7 个业务字段，`T-020` 和上一轮 bugfix 已要求若依默认能力进入审计。
- **为什么仍然犯错**：旧经验没有写成具体审计项：验证码禁用、四县分类下拉联动、多组联系人但不新增展示字段、H5 返回上下文均缺少可执行测试。
- **修复**：登录改为账号密码登录并关闭初始化验证码配置；内容表单改为办公室优先，四县分类锁定服务指引，后端按县域服务指引归一保存；产品新增/编辑支持 `contacts` 最多 5 组并兼容旧单字段，详情仍按业务经办人/联系方式两项展示；H5 详情链接携带返回上下文并由详情页解析返回目标；PRD/API/Plan 和审计脚本同步补强。
- **避坑规则**：若依默认登录、注册、验证码等模板能力即使入口隐藏也要进构建和审计；表单联动不能只靠提交时报错，前端可选项和后端归一/校验要同时覆盖；当用户要求多组录入但原型字段不增加时，必须明确“录入结构”和“展示契约”如何兼容；H5 详情返回必须保存来源上下文，不能依赖默认首页状态。

### Bugfix: 金融产品多联系人按钮、Jackson 运行时依赖与 H5 配对展示
- **触发**：用户反馈新增金融产品页面点击新增业务经办人后减号按钮被挤变形；新增/编辑确认提交返回 `NoClassDefFoundError: com/fasterxml/jackson/core/util/InternalJacksonUtil`；并询问 H5 多联系人是否也要对应增加展示。
- **根因**：若依产品表单把两个圆形按钮直接放在 `36px` 网格列中，未清理 Element 相邻按钮默认间距；`ruoyi-common` 只显式声明 `jackson-databind`，缺少对同版本 `jackson-core`/`jackson-annotations` 的可执行约束，旧运行 jar 也可能继续暴露依赖错配；H5 产品详情只把多行经办人和电话分别渲染为两个段落，未按行配对。
- **已有经验回查**：有，上一条多联系人 bugfix 已要求“不新增产品字段”和“最多 5 组”，`T-014` 已要求金融产品保持 7 个业务字段。
- **为什么仍然犯错**：旧经验没有落成按钮固定尺寸、fat jar 依赖检查、H5 多联系人配对展示的测试和审计规则。
- **修复**：若依产品表单增加固定 `product-contact-actions` 动作区，按钮固定 32px；`ruoyi-common` 显式声明 Jackson 三件套并测试 `InternalJacksonUtil` 可加载；后台 Controller 测试覆盖 `contacts` 数组解析；H5 使用 `splitProductContacts` 按换行顺序配对展示多组联系人；重打包并确认 fat jar 内含 `jackson-core-2.19.4.jar`。
- **避坑规则**：动态表单中的图标按钮必须有稳定容器、固定宽高和移动端布局检查；涉及 Jackson/Spring Boot 版本升级时，运行时三件套必须显式同版本，并检查 fat jar `BOOT-INF/lib`；多组联系人压缩到两个契约字段时，H5 只做配对展示，禁止借机新增贷款额度、利率等原型外字段。

### Bugfix: 后台业务账号路由权限与登录错误提示
- **触发**：用户反馈新建账号登录后菜单只有首页，`ry` 账号菜单也不符合预期；输错密码时页面展示“系统未知错误，请反馈给管理员”，而接口真实返回“用户不存在/密码错误”。
- **根因**：账号管理服务只写入 `sys_user` 和 `cb_account_extension`，没有同步若依 `sys_user_role`，导致 `/getRouters` 无法从若依角色菜单体系拿到业务菜单；同时若依前端统一请求封装优先使用默认错误字典和 `msg`，没有优先展示本项目契约字段 `message`。
- **已有经验回查**：`T-016` 已要求业务角色和办公室由 `cb_account_extension` 承载，`T-020` 已要求若依模板入口进审计，`T-009` 已要求认证失败按 401/403 契约返回。
- **为什么仍然犯错**：旧经验没有明确“业务账号扩展必须桥接若依原生角色关系”，审计也没有覆盖新建账号后的 `sys_user_role`、`/getRouters` 菜单过滤和前端错误 message 优先级。
- **修复**：新建业务账号同步写入公共业务角色 `role_id=2`，删除账号同步清理角色关系；历史账号通过 SQL 补齐角色桥接；`SysLoginController` 按 `cb_account_extension` 过滤业务菜单和按钮权限；登录失败补 401 异常处理；`request.js` 与下载插件优先展示响应体 `message`。
- **避坑规则**：若依基座上的业务账号不能只写业务扩展表，必须同时维护 `sys_user_role` 并用测试覆盖；业务菜单可见性必须同时经过若依角色菜单和项目业务角色过滤；统一响应以 `message` 为准，默认错误字典只能作为最后兜底，不能覆盖后端真实提示。

### Bugfix: 后台内容办公室锁定与首页快捷入口权限
- **触发**：用户反馈非管理员新增内容时应默认选中对应办公室且不可修改；首页快捷操作没有对应权限时应直接不可点击，不能点击后进入 404。
- **根因**：内容管理页面只做了办公室选择后的分类联动，没有读取当前登录账号业务扩展并锁定办公室；工作概览快捷入口使用无条件路由跳转，依赖路由权限兜底；`/getInfo` 未返回 `account_extension`，前端 Store 缺少当前业务角色和办公室上下文。
- **已有经验回查**：`T-013` 已要求普通账号只能管理本办公室内容，`T-018` 已要求工作概览只保留快捷入口，上一条 bugfix 已要求业务菜单和权限过滤。
- **为什么仍然犯错**：旧经验把重点放在服务端权限和菜单可见性，没有把字段禁用态、默认值和快捷入口可点击状态列为权限体验验收项。
- **修复**：`/getInfo` 增加 `account_extension`；Vuex 保存当前账号业务扩展；内容新增/编辑弹窗对非管理员默认并锁定本人办公室；工作概览快捷操作按 `centralbank:*` 按钮权限计算 enabled，无权限按钮禁用并阻止跳转；PRD/API/Plan/澄清文档和审计脚本同步补强。
- **避坑规则**：权限控制不能只靠菜单隐藏、路由 404 或服务端 403；表单默认值、字段禁用态和快捷入口是否可点击必须同步按权限处理。前端需要办公室级体验约束时，当前用户上下文必须显式返回业务扩展信息。

### Bugfix: 后台 5176 验收端口 CORS 漏配
- **触发**：用户反馈“登录后台返回Invalid CORS request”。
- **根因**：本地同时启动 H5 和若依后台时，H5 占用 `5175`，后台改用 `5176`；后端 CORS 白名单和启动文档仍只覆盖旧端口集合，导致浏览器携带 `Origin: http://127.0.0.1:5176` 时被 `CorsFilter` 拒绝。
- **已有经验回查**：已有 `T-009` 与后台登录相关 bugfix 经验覆盖登录契约、验证码禁用、错误提示和权限桥接，但没有覆盖并行验收端口变化。
- **为什么仍然犯错**：启动时临时调整端口后，只验证了页面和代理路径，没有把新 Origin 加入 CORS 回归测试，也没有同步 `docs/startup.md`。
- **修复**：`ResourcesConfig` 增加 `http://localhost:5176` 与 `http://127.0.0.1:5176`；`SecurityContractTest` 增加 5176 断言；`docs/startup.md` 将若依后台本地用户验收端口同步为 `5176`。
- **避坑规则**：前端端口只要因并行启动、冲突避让或用户验收而变化，必须同时更新 CORS 白名单、启动文档和安全契约测试；验证时必须模拟真实浏览器 `Origin`，不能只测无 Origin 的 curl。
