...
- 一级 Tab：左侧 `乡村振兴`、右侧 `金融服务`，首次进入默认选中 `乡村振兴`，所有 H01 原型统一展示乡村振兴高亮态，使用浅蓝容器和高对比分段按钮。
...
- H5 首页列表不展示加载更多按钮；用户滚动接近底部时自动加载下一页，若首屏内容不足以撑满视口则自动补页；无内容显示空状态，请求失败显示重试入口。
...
- 当后台录入多组业务经办人与联系方式时，H5 仍只使用“业务经办人”和“联系方式”两个契约字段，按换行顺序配对展示多组联系人。
...
- 后台登录页不展示验证码，登录请求只提交账号和密码。
- 登录失败显示明确提示。
- 登录后进入工作概览或首个有权限菜单。
- 工作概览展示内容、产品、账号和当日操作统计，并提供快捷入口；没有对应功能权限的快捷入口不可点击。
...
- 新增或编辑时先选择发布办公室、再选择分类；肇州县、肇源县、林甸县、杜蒙县只能选择服务指引。
- 非管理员新增或编辑内容时，发布办公室默认选中本人所属办公室且不可修改。
...
- 业务经办人与联系方式可添加多组，最多 5 组，默认第一组不可删除；H5 与接口仍只展示“业务经办人”和“联系方式”两项。
- Excel 导入先校验后提交；错误行不写入。
- 首次上线参考 Excel 来源共 112 条产品：涉农产品 52 条，小微产品 60 条。T-019 本轮按用户确认先抽取 10 条真实样本完成初始化闭环，不声明 112 条全量已初始化。
...
| `business_manager` | String | 业务经办人；多组时按行分隔 |
| `contact_info` | String | 联系方式；多组时按行分隔 |
...
- 初始产品数据来源共 112 条：涉农产品 52 条，小微产品 60 条；T-019 本轮初始化 10 条真实样本，后续如需全量预置再单独推进。
